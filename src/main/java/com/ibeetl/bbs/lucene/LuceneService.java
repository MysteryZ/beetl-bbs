...
//					analyzerUseSmart = new IKAnalyzer5x(true);
	...
	public Analyzer getAnalyzerNotUseSmart() {
    ...
    private void saveBbsIndexList(List<BbsIndex> bbsIndexList) {
		...
		try {
			...
			for (BbsIndex t : bbsIndexList) {
				...
				// storedFiled默认存储
				Field id = new StoredField("id", t.getId());
		...
		try {
			...
			for (ScoreDoc scoreDoc : topDocs.scoreDocs) {
				...
				if (document.get("indexType").equals("1")) { // 主题贴
					...
					searchResults.add(new IndexObject(topic.getId(),
							topic.getIsUp(),topic.getIsNice(),
							user, 
							new Date(Long.valueOf(document.get("createTime"))),
							topic.getPostCount(),topic.getPv(),
							module, content,
							firstPost!=null?firstPost.getContent():"", 1, score));
					...
					searchResults.add(new IndexObject(topic.getId(),
							...
							topic.getPostCount(),topic.getPv(),
							module,
							topic.getContent(), content, 2, score));
				}
